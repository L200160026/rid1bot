...
const { downloader, cekResi, removebg, urlShortener, meme, translate, getLocationData, yts } = require('../../lib')
...
module.exports = msgHandler = async (client = new Client(), message) => {
    try {
        ...
        console.log(args);
        console.log(isCmd);
        console.log(uaOverride);
        console.log(url);
        console.log(isQuotedImage);
        ...
        case 'pantun': {
          ...
          fetch('https://raw.githubusercontent.com/pajaar/grabbed-results/master/pajaar-2020-pantun-pakboy.txt')
            ...
            .then(body => {
              let tod = body.split("\n");
              let pjr = tod[Math.floor(Math.random() * tod.length)];
              client.sendText(from, pjr.replace(/pjrx-line/g,"\n"));
            });
            break
        }
        case 'quotes': {
        ...
        case 'play': {
          if (args.length == 0) return client.reply(from, 'Maaf, format pesan salah silahkan periksa menu. [Wrong Format]', id)
          var keyword = arg.replace(/ /g, "+");
          ...
          (async () => {
            ...
            YD.on("finished", function(err, data) {
              ...
              client.sendFileFromUrl(from, data.thumbnail, 'photo.jpg', caption, null, null, true)
                        .then((serialized) => console.log(`Sukses Mengirim File dengan id: ${serialized} diproses selama ${processTime(t, moment())}`))
                        .catch((err) => console.error(err));

              client.sendFile(from, data.file, 'video.mp3', '', null, true)
                        .then((serialized) => console.log(`Sukses Mengirim File dengan id: ${serialized} diproses selama ${processTime(t, moment())}`))
                        .catch((err) => console.error(err))
            });
